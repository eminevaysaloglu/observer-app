...
function Edit() {
    ...
    async function getData() {
        ...
        const response = await service.getObserverDriverAccessPrivilege(params.id)
        ...
        const res_observer_driver = await service.getObserverDrivers(p)
        newAutoOptions["observerDrivers"] = res_observer_driver?.data?.data
    ...
    async function updateFormData() {
        ...
        const result = await service.updateObserverDriverAccessPrivilege(payload)
        if ([200, 201].includes(result?.status)) {
            setFieldsData(fields())
            closeDrawer()
            NotificationManager.success("Gözlemci Sürücü Erişimi  başarılı bir şekilde güncellendi")
        } else {
            NotificationManager.error("Gözlemci Sürücü Erişimi  güncellenirken bir hata ile karşılaşıldı")
        }
    }

    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Gözlemci Sürücü Erişimi Düzenle">
                {
                    isLoading ?
                        <CircularProgress /> :
