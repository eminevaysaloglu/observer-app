...
import { columns } from '../../constants/observer_drivers/table_columns_data'
import observerDriverService from '../../service';
...
function ObserverDrivers() {
    ...
    useEffect(() => {
        const column = {
          ...
          getActions: (params) => [
            <ButtonPermission
              permission="TE_OBSERVER_DRIVERS_UPDATE"
              grid
              icon={<EditIcon />}
              onClick={() => editObserverDriver(params)}
              label="Edit"
            ></ButtonPermission>,
            <ButtonPermission
              permission="TE_OBSERVER_DRIVERS_DELETE"
              grid
              icon={<DeleteIcon />}
              onClick={() => deleteObserverDriver(params)}
              label="Delete"
            ></ButtonPermission>
          ]
        }
    ...
    function editObserverDriver(params) {
        const observerDriverId = params.id
        history.push(`/telematics/observers/observer-driver/edit/${observerDriverId}`)
    }

    async function deleteObserverDriver(params) {
        const observerDriverId = params.id
        const result = await service.deleteObserverDriver(observerDriverId)
        if (result?.status === 200) {
            getData()
            NotificationManager.success("Gözlemci Sürücü İlişkisi silindi")
        } else {
            NotificationManager.error("Gözlemci Sürücü İlişkisi silinirken bir hata oluştu")
        }
    }
    ...
    async function getData() {
        ...
        const result = await observerDriverService.getObserverDrivers(params)
...
const properties = result?.data?.properties
    ...
    function goCreate() {
        history.push(`/telematics/observers/observer-driver/create`)
    }
        ...
        {isLoading ? (
          ...
          <div className="observer-drivers-list-table-area">
            <div className="observer-drivers-list-table-area-header">
              <ButtonPermission
                permission="TE_OBSERVER_DRIVERS_CREATE"
                variant="outlined"
                startIcon={<Add />}
                size="small"
                onClick={goCreate}
...
export default ObserverDrivers
