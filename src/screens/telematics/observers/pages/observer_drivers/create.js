...
function Create() {
    ...
    async function saveFormData() {
        ...
        const result = await service.saveObserverDriver(payload)
        if ([200, 201].includes(result?.status)) {
            setFieldsData(fields())
            closeDrawer()
            NotificationManager.success("Gözlemci Sürücü İlişkisi başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Gözlemci Sürücü İlişkisi kayıt edilirken bir hata ile karşılaşıldı")
        }
    ...
    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Gözlemci Sürücü İlişkisi Ekle">
