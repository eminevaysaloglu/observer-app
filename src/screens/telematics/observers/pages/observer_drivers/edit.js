...
import { fields } from '../../constants/observer_drivers/create_fields'
...
function Edit() {
    ...
    async function getData() {
        ...
        const response = await service.getObserverDriver(params.id)
        ...
        const res_observer = await service.getObservers(p)
    ...
    async function updateFormData() {
        ...
        const result = await service.updateObserverDriver(payload)
        if ([200,201].includes(result?.status)) {
            setFieldsData(fields())
            closeDrawer()
            NotificationManager.success("Gözlemci Sürücü İlişkisi  başarılı bir şekilde güncellendi")
        } else {
            NotificationManager.error("Gözlemci Sürücü İlişkisi  güncellenirken bir hata ile karşılaşıldı")
        }
    }

    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Gözlemci Sürücü İlişkisi Düzenle">
                {
                    isLoading ?
                        <CircularProgress /> :
