...
function Create() {
    ...
    useEffect(async () => {
        ...
        const res = await service.getObserverVehicles(params)
        newAutoOptions["observerVehicles"] = res?.data?.data
    ...
    async function saveFormData() {
        ...
        if ([200, 201].includes(result?.status)) {
            ...
            closeDrawer()
            NotificationManager.success("Gözlemci Araç Erişimi başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Gözlemci Araç Erişimi kayıt edilirken bir hata ile karşılaşıldı")
        }
    ...
    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Gözlemci Araç Erişimi Ekle">
