...
function Create() {
    ...
    async function saveFormData() {
        ...
        const result = await service.saveEntityGroup(payload)
        if ([200,201].includes(result?.status)) {
            setFieldsData(fields())
            closeDrawer()
            NotificationManager.success("Varlık Grubu başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Varlık Grubu kayıt edilirken bir hata ile karşılaşıldı")
        }
    ...
    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Varlık Grubu Ekle">
