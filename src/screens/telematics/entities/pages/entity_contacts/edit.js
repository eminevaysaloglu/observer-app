...
function Edit() {
    ...
    async function getData() {
        ...
        const response = await service.getEntityContact(params.id)
    ...
    async function updateFormData() {
        ...
        const result = await service.updateEntityContact(payload)
        if ([200,201].includes(result?.status)) {
            setFieldsData(fields())
            closeDrawer()
            NotificationManager.success("Varlık İletişimi başarılı bir şekilde güncellendi")
        } else {
            NotificationManager.error("Varlık İletişimi güncellenirken bir hata ile karşılaşıldı")
        }
    }

    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Varlık İletişimi Düzenle">
