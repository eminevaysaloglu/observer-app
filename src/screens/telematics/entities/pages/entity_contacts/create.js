...
import { fields } from '../../constants/entity_contacts/create_fields'
...
function Create() {
    ...
    useEffect(async () => {
        ...
        const res = await service.getEntities(params)
        newAutoOptions["entities"] = res?.data?.data
    ...
    async function saveFormData() {
        ...
        const result = await service.saveEntityContact(payload)
        if ([200,201].includes(result?.status)) {
            setFieldsData(fields())
            closeDrawer()
            NotificationManager.success("Varlık İletişimi başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Varlık İletişimi kayıt edilirken bir hata ile karşılaşıldı")
        }
    ...
    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Varlık İletişimi Ekle">
