...
function Create() {
    ...
    useEffect(async () => {
        ...
        const res = await service.getEntities(p)
    ...
    async function saveFormData() {
        ...
        const result = await service.saveEntityGroupRelation(payload)
        if ([200, 201].includes(result?.status)) {
            setFieldsData(fields())
            closeDrawer()
            NotificationManager.success("Varlık Grup İlişkisi başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Varlık Grup İlişkisi kayıt edilirken bir hata ile karşılaşıldı")
        }
    ...
    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Varlık Grup İlişkisi Ekle">
