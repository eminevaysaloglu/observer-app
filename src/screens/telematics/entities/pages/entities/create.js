...
function Create() {
    ...
    useEffect(async () => {
        ...
        const res = await service.getEntityTypes(params)
        newAutoOptions["types"] = res?.data?.data
        const res_user = await service.getUsers(params)
    ...
    async function saveFormData() {
        ...
        const result = await service.saveEntity(payload)
        if ([200, 201].includes(result?.status)) {
            setFieldsData(fields())
            closeDrawer()
            NotificationManager.success("Varlık başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Varlık kayıt edilirken bir hata ile karşılaşıldı")
        }
    ...
    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Varlık Ekle">
