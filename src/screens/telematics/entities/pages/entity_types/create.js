...
function Create() {
    ...
    async function saveFormData() {
        ...
        const result = await service.saveEntityType(payload)
        if ([200, 201].includes(result?.status)) {
            setFieldsData(fields())
            closeDrawer()
            NotificationManager.success("Varlık Tipi başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Varlık Tipi kayıt edilirken bir hata ile karşılaşıldı")
        }
    ...
    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Varlık Tipi Ekle">
