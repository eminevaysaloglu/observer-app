...
import { fields } from '../../constants/entity_types/create_fields'
...
function Edit() {
    ...
    async function getData() {
        ...
        const response = await service.getEntityType(params.id)
    ...
    async function updateFormData() {
        ...
        const result = await service.updateEntityType(payload)
        if ([200,201].includes(result?.status)) {
            setFieldsData(fields())
            closeDrawer()
            NotificationManager.success("Varlık Tipi  başarılı bir şekilde güncellendi")
        } else {
            NotificationManager.error("Varlık Tipi  güncellenirken bir hata ile karşılaşıldı")
        }
    }

    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Varlık Tipi Düzenle">
                {
                    isLoading ?
                        <CircularProgress /> :
