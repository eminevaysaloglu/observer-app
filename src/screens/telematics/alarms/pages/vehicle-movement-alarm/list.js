...
import { columns } from '../../constants/vehicle-movement-alarm/table_columns_data'
import vehicleMovementAlarmsServices from '../../service';
...
function VehicleMovementAlarms() {
  ...
  const [vehicleMovementAlarms, setVehicleMovementAlarms] = useState([])
  ...
  useEffect(() => {
    const column = {
      ...
      getActions: (params) => [
        <ButtonPermission
          permission="TE_VEHICLE_MOVEMENT_ALARMS_UPDATE"
          grid
          icon={<EditIcon />}
          onClick={() => editVehicleMovementAlarms(params)}
          label="Edit"
        ></ButtonPermission>
      ]
    }
  ...
  function editVehicleMovementAlarms(params) {
    const id = params.id
    history.push(`${BASE_PATH}vehicle-movement-alarm/edit/${id}`)
  }
  ...
  async function getData() {
    ...
    const result = await vehicleMovementAlarmsServices.getVehicleMovementAlarms(params)
    const data = result?.data?.data
const properties = result?.data?.properties
    setCount(properties.totalPages)
    setVehicleMovementAlarms(data)
    setIsLoading(false)
  }
  ...
  function goCreate() {
    history.push(`${BASE_PATH}vehicle-movement-alarm/create/`)
  }
      ...
      {isLoading ? (
        ...
        <div className="alarms-list-table-area">
          <div className="alarms-list-table-area-header">
            <ButtonPermission
              permission="TE_VEHICLE_MOVEMENT_ALARMS_CREATE"
              variant="outlined"
              startIcon={<Add />}
              size="small"
              onClick={goCreate}
            >
              Yeni Hareket Alarmı Ekle
            </ButtonPermission>
          </div>

          <DataGrid
            components={{ Toolbar: Header, NoRowsOverlay: CustomNoRowsOverlay, FilterPanel: CustomFilterPanel }}
            rows={vehicleMovementAlarms}
            columns={columns}
            rowHeight={35}
            headerHeight={35}
            autoHeight
...
export default VehicleMovementAlarms
