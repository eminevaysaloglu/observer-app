...
function Create() {
    ...
    async function saveFormData() {
       ...
       /*  setValidation(true)
        ...
        const result = await service.saveVehicleMovementAlarms(payload)

        if ([200,201].includes(result?.status)) {
            setFieldsData(fields())
            NotificationManager.success("Hareket alarmı başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Hareket alarmı eklenirken bir hata oluştu")
        }
    ...
    return (
      <div>
        <TDrawer closeDrawer={closeDrawer} title="Hareket Alarmı Ekle">
