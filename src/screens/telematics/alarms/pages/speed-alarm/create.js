...
function Create() {
  ...
  async function saveFormData() {
    ...
    const result = await service.saveSpeedAlarms(payload)

    if ([200, 201].includes(result?.status)) {
      setFieldsData(fields())
      NotificationManager.success('Hız alarmı başarılı bir şekilde kayıt edildi')
    } else {
      NotificationManager.error('Hız alarmı eklenirken bir hata oluştu')
    }
  ...
  return (
    <div>
      <TDrawer closeDrawer={closeDrawer} title="Hız Alarmı Ekle">
