...
function Create() {
    ...
    async function saveFormData() {
       ...
       /*  setValidation(true)
        ...
        const result = await service.saveVehicleIgnitionAlarms(payload)

        if ([200,201].includes(result?.status)) {
            setFieldsData(fields())
            NotificationManager.success("Kontak alarmı başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Kontak alarmı eklenirken bir hata oluştu")
        }
    ...
    return (
      <div>
        <TDrawer closeDrawer={closeDrawer} title="Kontak Alarmı Ekle">
