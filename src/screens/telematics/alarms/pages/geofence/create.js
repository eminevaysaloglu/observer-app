...
function Create() {
    ...
    async function saveFormData() {
       ...
       /*  setValidation(true)
        ...
        const result = await service.saveGeofenceAlarms(payload)

        if ([200,201].includes(result?.status)) {
            setFieldsData(fields())
            NotificationManager.success("Bölge alarmı başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Bölge alarmı eklenirken bir hata oluştu")
        }
    ...
    return (
      <div>
        <TDrawer closeDrawer={closeDrawer} title="Bölge Alarmı Ekle">
