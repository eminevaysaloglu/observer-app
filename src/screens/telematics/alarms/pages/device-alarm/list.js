...
import { columns } from '../../constants/device-alarm/table_columns_data'
import deviceAlarmsServices from '../../service';
...
function DeviceAlarms() {
    ...
    useEffect(() => {
      const column = {
        ...
        getActions: (params) => [
          <ButtonPermission
            permission="TE_DEVICE_ALARMS_UPDATE"
            grid
            icon={<EditIcon />}
            onClick={() => editDeviceAlarms(params)}
            label="Edit"
          ></ButtonPermission>
        ]
      }
    ...
    function editDeviceAlarms(params) {
      const id = params.id
      history.push(`${BASE_PATH}device-alarm/edit/${id}`)
    }
    ...
    async function getData() {
        ...
        const result = await deviceAlarmsServices.getDeviceAlarms(params)
...
const properties = result?.data?.properties
    ...
    function goCreate() {
      history.push(`${BASE_PATH}device-alarm/create/`)
    }
        ...
        {isLoading ? (
          ...
          <div className="alarms-list-table-area">
            <div className="alarms-list-table-area-header">
              <ButtonPermission
                permission="TE_DEVICE_ALARMS_CREATE"
                variant="outlined"
                startIcon={<Add />}
                size="small"
                onClick={goCreate}
              >
                Yeni Cihaz Alarmı Ekle
              </ButtonPermission>
            </div>
