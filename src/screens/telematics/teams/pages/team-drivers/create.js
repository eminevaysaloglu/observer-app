...
function Create() {
    ...
    useEffect(async () => {
        ...
        newAutoOptions["drivers"] = res?.data?.data
    ...
    async function saveFormData() {
        ...
        const result = await service.saveTeamDrivers(payload)

        if ([200, 201].includes(result?.status)) {
            setFieldsData(fields())
            NotificationManager.success("Takım sürücüsü başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Takım sürücüsü kaydedilirken bir hata oluştu")
        }
    ...
    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Takım Sürücüsü Ekle">
