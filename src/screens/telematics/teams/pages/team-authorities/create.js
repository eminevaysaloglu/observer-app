...
function Create() {
    ...
    async function saveFormData() {
        ...
        const result = await service.saveTeamAuthorities(payload)

        if ([200, 201].includes(result?.status)) {
            setFieldsData(fields())
            NotificationManager.success("Takım yetkilisi başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Takım yetkilisi kaydedilirken bir hata oluştu")
        }
    ...
    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Takım Yetkilisi Ekle">
