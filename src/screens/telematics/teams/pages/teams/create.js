...
function Create() {
    ...
    async function saveFormData() {
        ...
        const result = await service.saveTeams(payload)
        if ([200, 201].includes(result?.status)) {
            setFieldsData(fields())
            NotificationManager.success("Takım başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Takım Cihaz kaydedilirken bir hata oluştu")
        }
        setIsLoading(false)
        closeDrawer()
    }
    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Takım Ekle">
