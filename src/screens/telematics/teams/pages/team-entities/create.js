...
function Create() {
    ...
    useEffect(async () => {
        ...
        const res_entitiy = await service.getEntities(params)
    ...
    async function saveFormData() {
        ...
        const result = await service.saveTeamEntities(payload)

        if ([200, 201].includes(result?.status)) {
            setFieldsData(fields())
            NotificationManager.success("Başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Kaydedilirken bir hata oluştu")
        }
    ...
    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Takım Varlığı Ekle">
