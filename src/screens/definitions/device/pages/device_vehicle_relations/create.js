...
function Create() {
    ...
    async function saveFormData() {
        ...
        const result = await service.saveDeviceVehicleRelations(payload)

        if ([200, 201].includes(result?.status)) {
            setFieldsData(fields())
            NotificationManager.success("Cihaz- araç ilişkisi başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error('Cihaz- araç ilişkisi kaydedilirken bir hata oluştu')
        }
    ...
    return (
      <div>
        <TDrawer closeDrawer={closeDrawer} title="Cihaz- araç ilişkisi Ekle">
