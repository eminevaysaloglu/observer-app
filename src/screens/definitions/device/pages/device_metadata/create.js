...
function Create() {
    ...
    useEffect(async () => {
        ...
        const resKeys = await service.getDeviceMetadataKeys(params)
    ...
    async function saveFormData() {
        ...
        const result = await service.saveDeviceMetadata(payload)
        if ([200,201].includes(result?.status)) {
            setFieldsData(fields())
            closeDrawer()
            NotificationManager.success("Cihaz metadata başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Cihaz metadata kayıt edilirken bir hata ile karşılaşıldı")
        }
