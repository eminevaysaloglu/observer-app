...
import { fields } from '../../constants/device_metadata/create_fields'
...
function Edit() {
    ...
    async function getDeviceMetadata() {
        ...
        const response = await service.getDeviceMetadata(params.id)
    ...
    async function updateFormData() {
        ...
        const result = await service.updateDeviceMetadata(params.id, payload)
        if ([200,201].includes(result?.status)) {
            setFieldsData(fields())
            closeDrawer()
            NotificationManager.success("Cihaz metadata başarılı bir şekilde güncellendi")
        } else {
            NotificationManager.error("Cihaz metadata güncellenirken bir hata ile karşılaşıldı")
        }
    }
