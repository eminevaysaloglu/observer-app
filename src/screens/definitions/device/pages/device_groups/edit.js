...
function Edit() {
    ...
    async function getDeviceGroups() {
        ...
        const response = await service.getDeviceGroups(params.id)
    ...
    async function updateFormData() {
        ...
        const result = await service.updateDeviceGroups(params.id, payload)
        if ([200,201].includes(result?.status)) {
            NotificationManager.success("Cihaz grubu başarılı bir şekilde güncellendi")
            closeDrawer()
        } else {
            NotificationManager.error("Cihaz grubu eklenirken bir düzeltilirken oluştu")
        }
    }

    return (
        <div>
            <TDrawer closeDrawer={closeDrawer} title="Cihaz Grubu Düzenle">
                {
                    isLoading ?
                        <CircularProgress /> :
