...
function Create() {
    ...
    useEffect(async () => {
      ...
      const res = await service.getNotificationMethods(params)
    ...
    async function saveFormData() {
       ...
       /*  setValidation(true)
        ...
        const result = await service.saveNotificationMethodProperties(payload)

        if ([200,201].includes(result?.status)) {
            setFieldsData(fields())
            NotificationManager.success("Bildirim yöntem özelliği başarılı bir şekilde kayıt edildi")
        } else {
            NotificationManager.error("Bildirim yöntem özelliği eklenirken bir hata oluştu")
        }
