...
function Create() {
  ...
  async function saveFormData() {
    ...
    const result = await service.saveSystemEventNotificationSetting(payload)
    if ([200, 201].includes(result?.status)) {
      setFieldsData(fields())
      closeDrawer()
      NotificationManager.success('Sistem olay bildirimleri başarılı bir şekilde kayıt edildi')
    } else {
      NotificationManager.error('Sistem olay bildirimleri kayıt edilirken bir hata ile karşılaşıldı')
    }
  ...
  return (
    <div>
      <TDrawer closeDrawer={closeDrawer} title="Sistem Olay Bildirimi Ekle">
